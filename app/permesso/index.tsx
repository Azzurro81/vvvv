...
import { RESIDENCE_CATEGORIES, categoryHasUnreadNews } from '../../constants/residence_laws';

export default function PermessoFoldersScreen() {
    const router = useRouter();
    const [cats] = useState(RESIDENCE_CATEGORIES);
    ...
    return (
       <>
           <Stack.Screen options={{ title: 'Permesso di Soggiorno' }} />
           <ScrollView style={styles.container} contentContainerStyle={styles.listContent}>
           <Text style={styles.sectionTitle}>Cosa devi fare con il tuo permesso?</Text>
              ...
              {cats.map(cat => {
                 ...
                 return (
                    <TouchableOpacity 
                       key={cat.id} 
                       style={styles.card} 
                       onPress={() => (router.push as any)(`/permesso/category/${cat.id}`)}
                    >
                       {hasNovelty && (
