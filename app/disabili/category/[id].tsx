...
import { DISABLED_CATEGORIES, DISABLED_LAWS, isLawUnread } from '../../../constants/disabled_laws';

export default function DisabiliCategoryLawsScreen() {
  ...
  const category = DISABLED_CATEGORIES.find(c => c.id === id);
  const laws = id === 'novita' ? DISABLED_LAWS.filter(l => l.isNew) : DISABLED_LAWS.filter(l => l.categoryId === id);
          ...
          renderItem={({ item }) => {
             ...
             return (
               <TouchableOpacity 
                   ...
                   onPress={() => (router.push as any)(`/disabili/law/${item.id}`)}
               >
