...
import { SANITA_CATEGORIES, SANITA_LAWS, isLawUnread } from '../../../constants/sanita_laws';

export default function SanitaCategoryScreen() {
  ...
  const category = SANITA_CATEGORIES.find(c => c.id === id);
  const laws = id === 'novita' ? SANITA_LAWS.filter(l => l.isNew) : SANITA_LAWS.filter(l => l.categoryId === id);
          ...
          renderItem={({ item }) => {
             ...
             return (
               <TouchableOpacity 
                  ...
                  onPress={() => (router.push as any)(`/sanita/law/${item.id}`)}
               >
