...
import { SANITA_CATEGORIES, categoryHasUnreadNews } from '../../constants/sanita_laws';

export default function SanitaFoldersScreen() {
    const router = useRouter();
    const [cats] = useState(SANITA_CATEGORIES);
    ...
    return (
       <>
           <Stack.Screen options={{ title: 'Salute e Sanità' }} />
           <ScrollView style={styles.container} contentContainerStyle={styles.listContent}>
           <Text style={styles.sectionTitle}>Guida alla Salute in Italia</Text>
              ...
              {cats.map(cat => {
                 ...
                 return (
                    <TouchableOpacity 
                       key={cat.id} 
                       style={styles.card} 
                       onPress={() => (router.push as any)(`/sanita/category/${cat.id}`)}
                    >
                       {hasNovelty && (
