...
export default function CartaLawDetailScreen() {
  ...
  const law = LONG_TERM_LAWS.find(l => l.id === id);
  ...
  return (
    <>
      <Stack.Screen options={{ title: 'Dettaglio Carta UE' }} />
      <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 80 }} showsVerticalScrollIndicator={true}>
        ...
        <View style={styles.contentPadding}>
            ...
            {law.documentsNeeded && (
            <>
                <Text style={styles.sectionTitle}>Documenti per il Kit</Text>
                {law.documentsNeeded.map((doc, i) => (
                    <View key={i} style={styles.bulletItem}>
                        <Ionicons name="reader-outline" size={18} color="#0ea5e9" style={{marginTop: 4, marginRight: 10}} />
