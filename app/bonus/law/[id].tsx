...
import { BONUS_LAWS, markLawAsRead } from '../../../constants/bonus_laws';
import { Ionicons } from '@expo/vector-icons';
import DocumentAccordion from '../../../components/DocumentAccordion';

import { useTranslatedLaw } from '../../../hooks/useTranslatedLaw';

export default function BonusLawDetailScreen() {
  const { id } = useLocalSearchParams();
  const initialLaw = BONUS_LAWS.find(l => l.id === id);
  ...
  if (!law) {
    return <View style={styles.container}><Text style={{padding: 20}}>Bonus non trovato</Text></View>;
  }
  ...
  return (
    <>
      <Stack.Screen options={{ title: isLoading ? 'Caricamento...' : 'Dettaglio Bonus' }} />
      <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 80 }} showsVerticalScrollIndicator={true}>
        ...
        <View style={styles.contentPadding}>
            ...
            {law.whereToApply && (
            <>
                <Text style={styles.sectionTitle}>Dove Fare Domanda</Text>
            ...
            <Text style={styles.sectionTitle}>Importo / Validità</Text>
